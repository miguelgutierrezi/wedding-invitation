...
import {
  isAdminNavActive,
  showAdminBackToFamilies,
  showAdminNewFamilyFab,
} from "@/lib/admin/admin-chrome";
...
export function AdminChrome({ title, email }: AdminChromeProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b-2 border-cover-cta-fg/15 bg-accent pt-[max(0.75rem,env(safe-area-inset-top))]">
        <div className="mx-auto flex w-full min-w-0 max-w-6xl flex-col gap-4 px-4 py-4 lg:gap-5 lg:px-8 lg:py-5">
      ...
      {showFab ? (
        <Link
          href="/admin/families/new"
          prefetch={false}
          className="fixed right-4 z-40 inline-flex size-14 items-center justify-center rounded-full bg-accent font-[family-name:var(--font-timer)] text-3xl leading-none text-white shadow-[0_8px_20px_rgba(69,68,17,0.16)] transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none active:scale-[0.98] lg:hidden bottom-[max(1.25rem,env(safe-area-inset-bottom))]"
