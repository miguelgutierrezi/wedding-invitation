-- Companion real names are required only when that guest will attend.
-- Lets a family decline a plus-one or any listed person without inventing a name.
...
:= null;
    if
v_guest_will_attend and coalesce(v_needs_name, false) then
