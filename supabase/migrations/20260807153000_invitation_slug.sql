...
create
or replace function public.slugify_label(input text)
returns text
language sql
immutable
as $$
...
alter table public.families
drop
constraint if exists families_invitation_slug_unique;
...
alter table public.families
drop
constraint if exists families_invitation_slug_format;
