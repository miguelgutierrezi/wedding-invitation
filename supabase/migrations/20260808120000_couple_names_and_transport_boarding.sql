-- Couple names + boarding point when a guest needs bus transport.

update public.events
set
  partner_one_name = 'Nychol',
  partner_two_name = 'Miguel',
  name = case
    when name ilike '%nombre 1%' or name ilike '%nombre 2%' then 'Matrimonio Nychol & Miguel'
    else name
  end,
  updated_at = timezone('utc', now());

alter table public.guests
  add column if not exists transport_boarding_point text;

alter table public.rsvp_response_guests
  add column if not exists transport_boarding_point text;

-- Allowed pickup keys (must match invitation transport.meetingPoints ids).
alter table public.guests
  drop constraint if exists guests_transport_boarding_point_valid;

alter table public.guests
  add constraint guests_transport_boarding_point_valid
  check (
    transport_boarding_point is null
    or transport_boarding_point in ('modelia', 'villa_sonia')
  );

alter table public.rsvp_response_guests
  drop constraint if exists rsvp_response_guests_transport_boarding_point_valid;

alter table public.rsvp_response_guests
  add constraint rsvp_response_guests_transport_boarding_point_valid
  check (
    transport_boarding_point is null
    or transport_boarding_point in ('modelia', 'villa_sonia')
  );
