...
create or replace function public.is_placeholder_guest_name(p_name text)
returns boolean
language sql
immutable
as $$
...
comment on function public.is_placeholder_guest_name(text) is
...
comment on column public.guests.needs_name_confirmation is
...
grant execute on function public.is_placeholder_guest_name(text) to service_role;
